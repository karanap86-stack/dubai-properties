  ...
  return (
    <div className="pt-24 pb-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-bold text-white mb-6 flex items-center gap-2"><Settings size={28} className="text-cyan-400" /> Social Integrations</h1>

        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 mb-6">
          <h2 className="text-lg font-semibold text-white mb-3 flex items-center gap-2"><Linkedin size={18} /> LinkedIn</h2>
          <p className="text-sm text-gray-400 mb-3">Provide your LinkedIn Page ID and Access Token (use backend for secure storage)</p>
          <div className="grid grid-cols-1 gap-3">
            <input value={configs.linkedin.pageId || ''} onChange={(e)=> handleChange('linkedin','pageId', e.target.value)} placeholder="LinkedIn Page ID" className="px-3 py-2 bg-slate-700 border border-slate-600 rounded text-white" />
            <input value={configs.linkedin.accessToken || ''} onChange={(e)=> handleChange('linkedin','accessToken', e.target.value)} placeholder="LinkedIn Access Token" className="px-3 py-2 bg-slate-700 border border-slate-600 rounded text-white" />
          </div>
        </div>

        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 mb-6">
          <h2 className="text-lg font-semibold text-white mb-3 flex items-center gap-2"><Image size={18} /> Instagram</h2>
          <p className="text-sm text-gray-400 mb-3">Provide Instagram Business Account ID and Access Token (backend required for direct posting)</p>
          <div className="grid grid-cols-1 gap-3">
            <input value={configs.instagram.accountId || ''} onChange={(e)=> handleChange('instagram','accountId', e.target.value)} placeholder="Instagram Account ID" className="px-3 py-2 bg-slate-700 border border-slate-600 rounded text-white" />
            <input value={configs.instagram.accessToken || ''} onChange={(e)=> handleChange('instagram','accessToken', e.target.value)} placeholder="Instagram Access Token" className="px-3 py-2 bg-slate-700 border border-slate-600 rounded text-white" />
          </div>
        </div>

        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 mb-6">
          <h2 className="text-lg font-semibold text-white mb-3 flex items-center gap-2"><Zap size={18} /> Auto-Posting</h2>
