const { getAllIndianStates, getCitiesByState } = require('./services/agentService')
const { getDevelopersByState, getDevelopersByCity } = require('./services/developerService')
// --- India Region & Developer API Endpoints ---
app.get('/api/india/states', (req, res) => {
  res.json({ success: true, states: getAllIndianStates() })
})

app.get('/api/india/cities', (req, res) => {
  const { state } = req.query
  if (!state) return res.status(400).json({ success: false, error: 'state required' })
  res.json({ success: true, cities: getCitiesByState(state) })
})

app.get('/api/india/developers', (req, res) => {
  const { state, city } = req.query
  if (state) return res.json({ success: true, developers: getDevelopersByState(state) })
  if (city) return res.json({ success: true, developers: getDevelopersByCity(city) })
  return res.status(400).json({ success: false, error: 'state or city required' })
})
// --- Developer/Project API Endpoints ---
app.post('/api/developers', (req, res) => {
  try {
    const dev = addDeveloper(req.body)
    res.json({ success: true, developer: dev })
  } catch (e) { res.status(500).json({ success: false, error: e.message }) }
})

app.get('/api/developers', (req, res) => {
  res.json({ success: true, developers: getAllDevelopers() })
})

app.post('/api/developers/:developerId/projects', (req, res) => {
  try {
    const project = addProjectToDeveloper(Number(req.params.developerId), req.body)
    res.json({ success: true, project })
  } catch (e) { res.status(500).json({ success: false, error: e.message }) }
})

app.get('/api/developers/:developerId/projects', (req, res) => {
  try {
    const projects = getProjectsByDeveloper(Number(req.params.developerId))
    res.json({ success: true, projects })
  } catch (e) { res.status(500).json({ success: false, error: e.message }) }
})

// TODO: Add endpoint to trigger content/marketing automation for a project or all projects
// --- Developer/Project Management & Auto-Refresh ---
const { addDeveloper, addProjectToDeveloper, getAllDevelopers, getProjectsByDeveloper } = require('./services/developerService')

// Example: Add a developer and project (in real use, expose API endpoints)
const demoDev = addDeveloper({ name: 'Prestige Group', region: 'India' })
addProjectToDeveloper(demoDev.id, { name: 'Prestige Lakeside Habitat', region: 'Bangalore', city: 'Bangalore', state: 'Karnataka' })

// TODO: Expose endpoints to manage developers/projects and trigger content/marketing automation
...
// Accepts: { ..., location: { country, locale }, preferredLanguage }
app.post('/api/leads', (req, res) => {
  const { location, preferredLanguage, ...rest } = req.body
  // Validate location and language
  const lead = {
    id: Date.now(),
    ...rest,
    location: location || null, // { country, locale }
      ...
      scheduledJobs[jobId] = schedule.scheduleJob(when, async () => {
        ...
        try {
          ...
          if (twilioClient && leadInfo && leadInfo.phone) {
            // India market: add India-specific notification logic or templates here if needed
            const body = `Reminder: ${leadInfo.name || 'Client'} - upcoming ${leadInfo.type || 'appointment'} at ${new Date(startISO).toLocaleString()}\nCountry: ${leadInfo.location?.country || ''}\nLanguage: ${leadInfo.preferredLanguage || ''}`
          ...
          if (nodemailer && process.env.SMTP_HOST) {
            ...
            // India market: add India-specific email templates or compliance here if needed
            const emailText = `Reminder: appointment at ${new Date(startISO).toLocaleString()}\nCountry: ${leadInfo.location?.country || ''}\nLanguage: ${leadInfo.preferredLanguage || ''}`
            await transporter.sendMail({ from: process.env.EMAIL_FROM || process.env.SMTP_USER, to: leadInfo?.email || process.env.SMTP_USER, subject: `Reminder: appointment`, text: emailText })
          }
  ...
  try {
    ...
    try {
      if (twilioClient && leadInfo?.phone) {
        // India market: add India-specific WhatsApp message logic here if needed
    ...
    try {
      if (nodemailer && process.env.SMTP_HOST) {
        const transporter = nodemailer.createTransport({ host: process.env.SMTP_HOST, port: process.env.SMTP_PORT || 587, secure: false, auth: { user: process.env.SMTP_USER, pass: process.env.SMTP_PASS } })
        // India market: add India-specific email templates or compliance here if needed
        const emailText = (message || `Reminder: upcoming ${type}`) + `\nCountry: ${leadInfo.location?.country || ''}\nLanguage: ${leadInfo.preferredLanguage || ''}`
        await transporter.sendMail({ from: process.env.EMAIL_FROM || process.env.SMTP_USER, to: leadInfo?.email || process.env.SMTP_USER, subject: `Reminder: ${type}`, text: emailText })
...
// Share lead with partner, including location and preferred language for global support
app.post('/api/share-lead', async (req, res) => {
  try {
    const { partnerId, partnerName, callbackUrl, lead, consent } = req.body
    // India market: add India-specific partner sharing logic or compliance here if needed
    // Ensure location and language are included in payload
    const payload = {
      partnerId,
      partnerName,
      lead: {
...
// Export analytics events, including location and preferred language for leads
app.post('/api/export-analytics-to-sheets', async (req, res) => {
  try {
    ...
    const auth = getGoogleAuthClient()
    // Enhance events: if type is 'lead_created', add country and language columns
    const values = events.map(e => {
      if (e.type === 'lead_created' && e.payload) {
        const country = e.payload.location?.country || ''
        const language = e.payload.preferredLanguage || ''
        return [e.timestamp, e.type, JSON.stringify(e.payload), country, language]
      }
      return [e.timestamp, e.type, JSON.stringify(e.payload)]
    })
    if (auth && google) {
      try {
        await auth.authorize()
        const sheets = google.sheets({ version: 'v4', auth })
        const sheetId = sheetLink.match(/\/d\/([a-zA-Z0-9-_]+)/)[1]
        await sheets.spreadsheets.values.append({ spreadsheetId: sheetId, range: 'Sheet1!A:E', valueInputOption: 'RAW', requestBody: { values } })
        return res.json({ ok: true })
